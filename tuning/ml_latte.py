...
if __name__ == '__main__':
    ...
    for r12, r3 in zip(grid[0].flatten(), grid[1].flatten()):
        ...
        for alpha in [2,3,4,5]:
            ...
            hr_tf[f'r={r12}_{r3}, alpha={alpha}'] = hr
            mrr_tf[f'r={r12}_{r3}, alpha={alpha}'] = mrr
            c_tf[f'r={r12}_{r3}, alpha={alpha}'] = C
    ...
    for params in exponential_attentions_list:
        ...
        for r12, r3 in zip(grid[0].flatten(), grid[1].flatten()):
            ...
            tf_recs = topn_recommendations(tf_scores, topn=10)
            for alpha in [2,3,4,5]:
                hr, hr_pos, hr_neg, mrr, mrr_pos, mrr_neg, cov, C = model_evaluate(tf_recs, holdout_valid,
                                                                                   data_description, alpha=alpha)
                hr_tf[f'r={r12}_{r3}, alpha={alpha}'] = hr
                mrr_tf[f'r={r12}_{r3}, alpha={alpha}'] = mrr
                c_tf[f'r={r12}_{r3}, alpha={alpha}'] = C
    print(c_tf)
